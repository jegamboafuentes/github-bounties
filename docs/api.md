...
Other `/api/v1` and MCP tool failures use the same envelope with `validation_failed`, `not_found`, or `internal`. Older `/api/*` routes keep `{ "ok": false, ... }`.
...
List rows include the issue, status, face (top-ups increase the face; `totalFundedUsdc` is that face), the empty-pool fee/pool schedule, cached badges, and the board funder stack (distinct count and up to five avatar URLs). The live roster split is on the detail route.
...
`GET /api/v1/bounties/{id}/funders` returns display name, GitHub login, avatar, amount, and time. It does not return email, Google subject, or wallet address.
