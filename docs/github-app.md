...
| Event | Why now | Later |
| --- | --- | --- |
| **Pull request** | Merge is truth. `action=closed` + `merged=true` is the eligibility signal **and** V2-2 pool freeze | `opened` / `synchronize` / `ready_for_review` ingest unfrozen roster candidates (UX only; freeze recomputes from GitHub) |
