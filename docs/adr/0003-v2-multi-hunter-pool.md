...
- **Status:** Accepted — V2-0 spike + V2-1 schema + V2-2 webhooks. Product rules frozen by LB PM (Enrique delegated).
...
- **V2-2 (this impl PR):** merge-time backfill freezes `pool_participants`. Incremental `opened` / `synchronize` / `ready_for_review` is UX only. No `settleEscrow` multi-payee, no UI, no exclusive claim-lock from this path.
...
> V2-3…V2-5 (multi-payee settle, UI, DEV dogfood) are still unimplemented. V1.5 WalletConnect + amount chips is
