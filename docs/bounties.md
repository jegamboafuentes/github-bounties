...
| `GET\|POST /api/jobs/poll-public-merges` | optional `CRON_SECRET` | Finds merged PRs that close funded issues on `public_reference` repos and writes the same Claim path as webhooks. Not scheduled by Cloud Build or Terraform |
...
**PROD (and DEV):** `cloudbuild.web.yaml` deploys the web service and does not create a Cloud Scheduler job. `infra/terraform` and `infra/gcloud` do not define one either. `CRON_SECRET` is not in Secret Manager and is not bound on the default deploy, so nothing in this repo starts the poller on a timer. The route ships with the web image: `GET` or `POST /api/jobs/poll-public-merges` runs a scan when something calls it. With `CRON_SECRET` unset the route is open, the same as expire-locks. An operator can also run `cd apps/web && npm run poll-public-merges` against that environment's database. Optional `GITHUB_PUBLIC_READ_TOKEN` raises the public REST rate limit; unauthenticated reads work without it.
