...
| `npm run poll-public-merges` | scan funded `public_reference` bounties for a merged PR that closes `#N` (same function as the cron route). Scheduled on DEV and PROD |
...
| URL | Auth |
| --- | --- |
| `/` | public homepage: live stats, differentiators, scroll story |
| `/about` | public origin page (Lightning Bounties lineage, cofounders, local photos). No session. |
| `/roadmap` | public roadmap (`PUBLIC_ROADMAP` / [docs/roadmap.md](../../docs/roadmap.md)). No session. |
...
| `GET\|POST /api/jobs/poll-public-merges` | optional `CRON_SECRET` bearer; DEV and PROD cron for Claim on public repos with no App install. PROD: Cloud Scheduler `gb-prod-poll-public-merges` every 10 minutes |
