...
| `npm run poll-public-merges` | scan funded `public_reference` bounties for a merged PR that closes `#N` (same function as the cron route). DEV only |
...
| `GET\|POST /api/jobs/poll-public-merges` | optional `CRON_SECRET` bearer; DEV cron for Claim on public repos with no App install |
