...
export function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  return ctx.params.then(({ id }) => handleV1Action(request, { kind: "signal", bountyId: id, method: "POST" }));
}

export function DELETE(request: Request, ctx: { params: Promise<{ id: string }> }) {
  return ctx.params.then(({ id }) => handleV1Action(request, { kind: "signal", bountyId: id, method: "DELETE" }));
}
