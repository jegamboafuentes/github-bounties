...
import { updateDisplayNameAction, updateNotificationPreferencesAction } from "@/app/actions/profile";
      ...
      <main className="mx-auto flex w-full max-w-xl flex-1 flex-col gap-8 px-6 py-14">
        ...
        <section className="flex flex-col gap-3 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            Shown on the board and in email. Saving a name here keeps it when you sign in with Google again.
          </p>
          <form action={updateDisplayNameAction} className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <input
              name="displayName"
              required
              maxLength={80}
        ...
        <section className="flex flex-col gap-3 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <form action={updateNotificationPreferencesAction} className="flex flex-col gap-2 text-sm">
            <label className="flex items-center gap-2">
              <input type="checkbox" name="bountyFunded" value="true" defaultChecked={emailPrefs.bountyFunded} />
              When a bounty you posted locks
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" name="prMerged" value="true" defaultChecked={emailPrefs.prMerged} />
              When your pull request wins a bounty
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" name="bountySettled" value="true" defaultChecked={emailPrefs.bountySettled} />
              When your winner payout is sent
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" name="poolClaimable" value="true" defaultChecked={emailPrefs.poolClaimable} />
              When a pool share is ready to claim
            </label>
            <button
              type="submit"
              className="mt-2 w-fit rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white dark:bg-zinc-100 dark:text-zinc-900"
            >
