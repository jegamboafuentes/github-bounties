import { closingIssueNumbersForRepo } from "./closing-keywords";
 ...
 * look at claim-lock state or the bounty table — callers intersect
 * `closedIssueNumbers` with funded issues.
 */
export function evaluateEligibility(
  ...
  if (!pr.merged) {
    return deny(repositoryFullName, "pull request closed without merge", pr);
  }

  if (!input.defaultBranch) {
    return deny(repositoryFullName, "repository default branch unknown", pr);
  }
  ...
  const fromText = closingIssueNumbersForRepo(
    [pr.title, pr.body, pr.mergeCommitMessage, ...(pr.commitMessages ?? [])],
    repositoryFullName,
  );
